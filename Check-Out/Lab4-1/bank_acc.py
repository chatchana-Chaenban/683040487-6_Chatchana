...
class BankAccount:
    ...
    def __init__(self, name, acc_type="saving", balance=0): # constructor
        ...
        if self.type == "saving": # saving type
           ...
           #type_code = 1
           self.running = f"{BankAccount.branch_number}-1-{BankAccount.saving_run}"
        
        elif self.type == "loan": # loaning type
           BankAccount.loaning_run += 1
           #type_code = 2
           self.running = f"{BankAccount.branch_number}-2-{BankAccount.loaning_run}"
    ...
    def print_customer(self):
       ...
       print(f"Account number: {self.running}")
